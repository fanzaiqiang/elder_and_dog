...
| 模組 | 進度 | 下一步 |
|------|------|--------|
| **Perception API** | 100% | DA3 + 區域分析完成，326ms 推論 ✅ |
| **YOLO-World 整合** | 10% | 物體偵測待整合 |
| **短期記憶系統** | 0% | 任務狀態機 + 目標追蹤 |
| **備案影片** | 0% | 錄製 Demo 備案 |

### 🎯 架構定位（12/16 更新）

**核心定位：** 輕量級雙系統架構（非端到端 VLA）

```
System 2 (慢系統): LLM 語意理解 + 決策規劃
      ↓ JSON 高階指令
System 1 (快系統): 確定性控制腳本 + Perception API
      ↓ ROS2 Services
     Go2 機器狗執行
```
...
- ✅ 給 LLM 吃「熟食」（DA3 JSON 摘要 → 1.5-2.5 秒延遲）
- ✅ 雙系統解耦：LLM 規劃 + 腳本執行，可靠性高
- ✅ 短期記憶：任務狀態機 + 目標追蹤

> 💡 **為什麼不用 VLA？** VLA 需要微調模型，我們採用零樣本 LLM 方案，更快部署、更可解釋、延遲更低。
