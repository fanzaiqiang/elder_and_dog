...
| **ros-mcp-server** | `ros-mcp-server/` | MCP 協定實作，LLM 透過此控制 ROS2 |
| **fsm_adapter_node** | `src/logic/fsm_adapter.py` | 解析 LLM 指令，轉換為 Nav2 Action 或 /cmd_vel |
...
### W7 (12/9-12/15)：視覺閉環 + FSM Adapter

**目標：** LLM 能看見環境並做出決策

| 日期 | 任務 | 驗收標準 |
|------|------|---------|
| 12/9-10 | 開發 snapshot_service | `/capture_snapshot` 回傳 base64 圖片 |
| 12/11-12 | 整合 YOLO 標註（可選） | 圖片上標示物件 bounding box |
| 12/13-14 | 開發 fsm_adapter_node | 訂閱 `/llm_command`，發送 Nav2 Action |
| 12/15 | 視覺閉環測試 | LLM 描述「我看見前方有障礙物」 |
...
| **Action 不支援** | 🔴 高 | FSM Adapter 轉接 | 僅用 /cmd_vel |
