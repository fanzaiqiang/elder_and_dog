...
**VM 環境變數（雙模式配置 - 2025/12/02 更新）：**

專案支援兩種 CycloneDDS 配置模式，根據使用場景自動切換：

| 模式 | 配置檔案 | 使用場景 | 切換方式 |
|------|---------|---------|---------|
| **開發模式** | `local_only_v2.xml` | VM 內部測試，單機 Demo | 預設模式 |
| **整合模式** | `cyclonedds_dual.xml` | Windows RViz2 控制，在家開發 | `export USE_WINDOWS_RVIZ2=1` |

**開發模式（預設）：**
```bash
# 直接啟動，使用 local_only_v2.xml（最穩定）
zsh phase1_test.sh t1

# 特點：
# ✅ 只走 loopback（127.0.0.1），VM 內部通訊絕對穩定
# ✅ 解決雙網卡 DDS 通訊問題
# ✅ 不干擾 Go2 WebRTC 連線（enp0s1 完全不參與 DDS）
# ❌ Windows RViz2 無法連線（但可用 Foxglove WebSocket）
```

**整合模式（Windows RViz2）：**
```bash
# 設定環境變數，使用 cyclonedds_dual.xml
export USE_WINDOWS_RVIZ2=1
zsh phase1_test.sh t1

# 特點：
# ✅ VM 內部通訊優先走 loopback（Priority 10）
# ✅ Windows RViz2 Native DDS 連線（< 1ms 延遲）
# ✅ 白名單策略：只開放 enp0s2（外部網卡）
# ✅ enp0s1（Go2 網卡）完全不參與 DDS，保護 WebRTC 連線
...
<Interfaces>
    ...
    <NetworkInterface name="enp0s2" priority="5"/>   <!-- 允許 Windows -->
    <!-- enp0s1 不列入 → 保護機器狗連線 -->
</Interfaces>
<Peers>
    <Peer address="127.0.0.1"/>        <!-- VM 內部 -->
    <Peer address="192.168.1.146"/>    <!-- Windows RViz2 -->
</Peers>
```

**詳細說明：** 請參考 `docs/01-guides/cyclonedds-config-guide.md`

---

#### 🚀 Phase 1 測試啟動流程（雙模式支援）
...
**場景 2：Windows RViz2 控制（在家開發）**

**Step 1: VM 啟動整合模式（Terminal 1）**
```bash
cd ~/ros2_ws/src/elder_and_dog
...
**Step 2: VM 啟動 SLAM/Nav2（Terminal 3）**
```bash
# 延續相同的環境變數設定
export USE_WINDOWS_RVIZ2=1
zsh phase1_test.sh t3
# 等待看到 "slam_toolbox: Message Filter dropping message..."
```
...
- ✅ TF 座標軸顯示（RGB 箭頭）
- ✅ Global Status: OK
- ✅ Windows `ros2 topic list` 可看到 `/scan`, `/map`, `/tf` 等 VM 節點發布的 Topics
...
在 11/30-12/02 的開發過程中，遇到兩個相互衝突的需求：

1. **11/30 目標**：Windows ↔ Mac VM Native DDS 連線，實現零延遲 RViz2 控制（< 1ms）
2. **12/02 問題**：VM 雙網卡（enp0s1 連 Go2 + enp0s2 連 Windows）導致 DDS 通訊混亂

**技術挑戰：**
- `enp0s1`（192.168.12.222）：必須穩定連接 Go2 的 WebRTC/SCTP 通訊
- `enp0s2`（192.168.1.200）：需要與 Windows（192.168.1.146）進行 DDS 通訊
- CycloneDDS 預設會嘗試使用所有網卡，導致 WebRTC 封包路由錯誤

**解決方案：白名單 + 優先級策略**

- **開發模式**：完全封閉 VM，只走 loopback，保證 WebRTC 絕對穩定
  - 用途：單機 Demo、學校展示、除錯時的最穩定配置
  - 限制：Windows RViz2 無法連線（可用 Foxglove WebSocket 替代）

- **整合模式**：精確控制網卡參與，保護 WebRTC 同時開放 Windows 通訊
  - 白名單只包含 `lo`（優先級 10）和 `enp0s2`（優先級 5）
  - `enp0s1`（Go2 網卡）**完全不參與 DDS**，避免干擾 WebRTC
  - VM 內部 Terminal 間通訊優先走 loopback（最快）
  - Windows RViz2 透過 `enp0s2` 建立 Native DDS 連線

**成果：**
- ✅ VM 內部通訊穩定（< 1ms）
- ✅ Windows RViz2 Native DDS 連線成功（< 1ms）
- ✅ Go2 WebRTC 連線不受影響（SCTP 握手成功）
- ✅ 可根據場景彈性切換，無需修改程式碼
...
| 問題 | 症狀 | 解決方案 |
|------|------|----------|
| **Windows RViz2 看不到 Topics** | `ros2 topic list` 只顯示 `/parameter_events`, `/rosout` | 1. **檢查 VM 是否啟用整合模式**：`echo $CYCLONEDDS_URI`（應顯示 `cyclonedds_dual.xml`）<br/>2. 如果顯示 `local_only_v2.xml`，請重啟 VM Terminals 並加上 `export USE_WINDOWS_RVIZ2=1`<br/>3. 確認 Windows `CYCLONEDDS_URI` 已設定<br/>4. 檢查防火牆 UDP 7400-7500 |
| **VM 內部 Topics 消失** | Terminal 2 `ros2 topic list` 看不到 `/scan`, `/map` | 1. 執行 `ros2 daemon stop`<br/>2. 確認所有 Terminal 使用相同的 `CYCLONEDDS_URI`<br/>3. 重啟所有 ROS2 節點 |
...
| **SCTP 握手超時** | `[go2_driver_node] ERROR - SCTP 握手超時` | 1. 確認 `enp0s1` 網卡沒有雙 IP（執行 `ip addr show enp0s1`）<br/>2. 執行 `phase1_test.sh env` 清洗網路配置<br/>3. 確認開發模式使用 `local_only_v2.xml`（DDS 不干擾 WebRTC） |
