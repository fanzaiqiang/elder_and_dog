...
tmux new-session -d -s go2_mcp -x 180 -y 40

# Step 1: 先垂直分割成上下兩列
tmux split-window -v -t go2_mcp:0

# Step 2: 上方 (pane 0) 水平分割成左右
tmux split-window -h -t go2_mcp:0.0

# Step 3: 下方 (pane 2，因為 split 後 index 變了) 水平分割成左右
tmux split-window -h -t go2_mcp:0.2

# 現在 pane 順序: 0=左上, 1=右上, 2=左下, 3=右下
...
# Pane 3 (右下): 測試終端
tmux send-keys -t go2_mcp:0.3 "sleep 15 && \
source /opt/ros/humble/setup.zsh && \
source ~/ros2_ws/install/setup.zsh && \
unset RMW_IMPLEMENTATION && unset CYCLONEDDS_URI && \
...
ros2 service list --no-daemon | grep snapshot && echo '' && \
echo '✅ MCP 系統就緒!'" C-m

# 連接到 session
echo -e "${GREEN}✅ 所有服務已啟動中...${NC}"
echo -e "${YELLOW}⏳ 請等待約 15 秒讓系統就緒...${NC}"
echo -e ""
echo -e "📋 使用說明："
...
echo -e "  測試 snapshot:  ${BLUE}ros2 service list | grep snapshot${NC}"
echo -e ""
